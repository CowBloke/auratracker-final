...
(function() {
  var $, HEXGL_SOURCE, a, defaultControls, getWebGL, hasWebGL, init, notifyParent, s, u, _fn, _i, _len;

  $ = function(_) {
    return document.getElementById(_);
  };

  HEXGL_SOURCE = 'aura-hexgl';
  ...
  notifyParent = function(type, payload) {
    ...
    return window.parent.postMessage(Object.assign({
      source: HEXGL_SOURCE,
      type: type
  ...
  init = function(controlType, quality, hud, godmode) {
    var hexGL, originalDisplayScore, progressbar;
    ...
    window.hexGL = hexGL;
    originalDisplayScore = hexGL.displayScore.bind(hexGL);
    hexGL.displayScore = function(finishTime, lapTimes) {
      var result;
      result = originalDisplayScore(finishTime, lapTimes);
      if (hexGL.gameplay != null) {
        notifyParent(hexGL.gameplay.result === hexGL.gameplay.results.FINISH ? 'finish' : 'destroyed', {
          score: finishTime / 1000,
          lapTimes: (lapTimes || []).map(function(time) {
            return time / 1000;
          })
        });
      }
