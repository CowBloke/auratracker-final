...
const pageMatchers: Array<{ test: (path: string) => boolean; label: string; icon: LucideIcon }> = [
  ...
  { test: (path) => path.startsWith('/you?tab=marche-actions'), label: 'Moi - Marche actions', icon: Coins },
  { test: (path) => path.startsWith('/you?tab=salle-de-marche'), label: 'Moi - Salle de marché', icon: Coins },
