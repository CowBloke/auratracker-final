...
import { riftApi, RiftTransaction, RiftPriceHistory } from '../services/api';
...
export default function Rift() {
  ...
  const [feePercentage, setFeePercentage] = useState(0.035);
  const [priceHistory, setPriceHistory] = useState<RiftPriceHistory[]>([]);
  const [riftBalance, setRiftBalance] = useState(0);
  ...
  const [myTransactions, setMyTransactions] = useState<RiftTransaction[]>([]);
  const [allTransactions, setAllTransactions] = useState<RiftTransaction[]>([]);
  
  const fetchData = useCallback(async () => {
    try {
      const [priceRes, myTxRes, allTxRes] = await Promise.all([
        riftApi.getPrice(24),
        riftApi.getMyTransactions({ limit: 50 }),
        riftApi.getAllTransactions({ limit: 50 }),
      ]);
      ...
      setRiftBalance(priceRes.data.userBalance.rift);
  ...
  useEffect(() => {
    ...
    socket.on('rift:price-update', handlePriceUpdate);
    
    return () => {
      socket.off('rift:price-update', handlePriceUpdate);
    };
  }, [socket]);
  ...
  const handleBuy = async () => {
    ...
    try {
      const res = await riftApi.buy(amount);
      setRiftBalance(res.data.newBalance.rift);
  ...
  const handleSell = async () => {
    ...
    try {
      const res = await riftApi.sell(amount);
      setRiftBalance(res.data.newBalance.rift);
      setMoneyBalance(res.data.newBalance.money);
      setCurrentPrice(res.data.transaction.newPrice);
      setSellAmount('');
    ...
    <div className="max-w-6xl mx-auto py-12 px-4 space-y-6">
      ...
      <div className="border border-border/30 p-6 space-y-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="border border-border/30 p-4">
            <p className="text-xs text-muted-foreground uppercase tracking-wide">Solde Rift</p>
            <p className="text-2xl font-light tabular-nums">{riftBalance.toFixed(4)} RFT</p>
            <p className="text-xs text-muted-foreground tabular-nums">
              ≈ ${(riftBalance * currentPrice).toFixed(2)}
            </p>
          </div>
          <div className="border border-border/30 p-4">
        ...
        <div className="grid md:grid-cols-2 gap-4">
            ...
            {buyMoneyAmount > 0 && (
              <div className="text-xs text-muted-foreground space-y-1 pt-1">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="tabular-nums text-foreground">{buyCoinsEstimate.toFixed(4)} RFT</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="border border-border/30 p-4 space-y-3">
            ...
            <div>
              <label className="text-xs text-muted-foreground">Quantité (RFT)</label>
              <div className="flex items-center gap-2 mt-1">
                <input
                  type="number"
                  value={sellAmount}
                ...
                <button
                  type="button"
                  onClick={() => setSellAmount(riftBalance.toFixed(4))}
                  disabled={loading || riftBalance <= 0}
                  className={cn(
                    "px-3 py-2 border text-[10px] uppercase tracking-widest transition-colors whitespace-nowrap",
                    !loading && riftBalance > 0
                ...
                <button
                  onClick={handleSell}
                  disabled={loading || !sellAmount || sellCoinAmount <= 0 || sellCoinAmount > riftBalance}
                  className={cn(
                    "px-4 py-2 border text-xs transition-colors whitespace-nowrap",
                    !loading && sellCoinAmount > 0 && sellCoinAmount <= riftBalance
              ...
              >
                ...
                <div className="text-right">
                  <p className="text-xs tabular-nums">
                    {tx.type === 'BUY' ? '+' : '-'}{tx.coinAmount.toFixed(4)} RFT
                  </p>
