...
interface TutorialGuide {
  ...
  content?: import('react').ReactNode | null;
  comingSoon?: boolean;
  tag?: string;
}
        ...
        <div className="space-y-2">
          {SKILLS_DATA.map((skill) => (
            <div key={skill.label} className="rounded-xl border border-border/40 bg-muted/5 px-4 py-3 space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                <strong className="text-foreground/80">Comment monter :</strong> {skill.how}
              </p>
              <p className="text-xs text-muted-foreground">
...
// ─── Newcomer guide subsections ───────────────────────────────────────────────

const newcomerGuideSubsections: TutorialSubsection[] = [
  {
    id: 'nc-welcome',
...
// ─── Clans guide subsections ──────────────────────────────────────────────────

const clanGuideSubsections: TutorialSubsection[] = [
  {
    id: 'clan-presentation',
    title: 'Présentation',
    content: (
      <div className="space-y-5">
        <p className={cn(TYPOGRAPHY.SMALL, 'leading-relaxed')}>
...
const tutorialGuides: TutorialGuide[] = [
  ...
  { id: 'ph-casino',  icon: ArrowLeftRight, title: 'Casino & jeux de hasard', description: "Crash, Mines, Roulette russe — comprendre les mécaniques, les risques et les limites de mise.",    tag: 'Jeux',       comingSoon: true },
];
...
function TutorialsTab() {
  ...
  const [selectedSubsectionId, setSelectedSubsectionId] = useState<string | undefined>(
    activeGuides[0].subsections?.[0]?.id,
  );

  const selectedGuide = tutorialGuides.find(g => g.id === selectedGuideId) ?? activeGuides[0];

  function handleGuideChange(id: string) {
    if (!id) return;
    const guide = tutorialGuides.find(g => g.id === id);
    if (!guide || guide.comingSoon) return;
          ...
          >
            {tutorialGuides.map((guide) => {
              ...
              return (
                ...
                >
                  <AccordionTrigger
                    className={cn('px-3 py-2 text-left hover:no-underline', isSelected && 'bg-primary/5')}
                  >
                    <div className="flex min-w-0 items-center gap-3">
                      <span
                        className={cn(
                          'flex h-7 w-7 shrink-0 items-center justify-center rounded-md border',
                          isSelected ? 'border-primary/20 bg-primary/10' : 'border-border/40 bg-muted/30',
                        )}
                      >
                        <Icon className="h-3.5 w-3.5 text-muted-foreground" />
                      </span>
