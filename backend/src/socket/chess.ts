...
import { AI_PLAYER_ID, AI_PLAYER_NAMES, AI_MOVE_DELAY_MS, aiPartyInfos, getAIChessMove, type AIDifficulty } from './ai-game-state.js';
...
async function endGame(game: ChessGame, io: Server, winnerId: string | null, result: ChessResult) {
  ...
  try {
    ...
    if (winnerId) {
      ...
      const cappedWinnerReward = winnerIsHuman ? await applyDailyGameRewardCaps(prisma, winner.userId, 'chess', resolvedWinnerReward) : null;
      const cappedLoserReward = loserIsHuman ? await applyDailyGameRewardCaps(prisma, loser.userId, 'chess', resolvedLoserReward) : null;
      ...
      const cappedDrawRewards = await Promise.all(
        humanPlayers.map(async (player) => {
          const capped = await applyDailyGameRewardCaps(prisma, player.userId, 'chess', {
