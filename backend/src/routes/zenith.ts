...
// Configuration
const INITIAL_PRICE = 62;
const FEE_PERCENTAGE = 0.028; // 2.8% fee
...
const initializePrice = async () => {
  try {
    const latestPrice = await prisma.zenithPrice.findFirst({
    ...
    if (latestPrice) {
      ...
      await prisma.zenithPrice.create({
        data: { price: INITIAL_PRICE, volume: 0 },
      });
    }
    console.log(`Zenith initialized at price: $${currentPrice.toFixed(2)}`);
  } catch (error) {
    console.error('Failed to initialize Zenith price:', error);
  }
};
...
export const startPriceEngine = () => {
  ...
  console.log('Zenith price engine started');
};
...
export const stopPriceEngine = () => {
  if (priceInterval) {
    ...
    priceInterval = null;
    console.log('Zenith price engine stopped');
  }
};
...
router.get('/price', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const history = await prisma.zenithPrice.findMany({
      where: {
    ...
    const user = await prisma.user.findUnique({
      ...
      select: { zenithBalance: true, money: true },
    });
...
router.post('/buy', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [updatedUser, transaction] = await prisma.$transaction([
      prisma.user.update({
        ...
        data: {
          ...
          zenithBalance: { increment: coinsReceived },
        },
      }),
      prisma.zenithTransaction.create({
        data: {
          userId: req.user.id,
          type: 'BUY',
    ...
    // Log buy
    logAuraCoin('zenith_buy', req.user.id, user.username, {
    ...
    res.json({
      ...
      newBalance: {
        money: updatedUser.money,
        zenith: updatedUser.zenithBalance,
      },
    });
  } catch (error) {
    console.error('Buy error:', error);
    res.status(500).json({ error: 'Failed to buy Zenith' });
  }
});
...
router.post('/sell', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (user.zenithBalance < coinAmount) {
      return res.status(400).json({ error: 'Insufficient Zenith balance' });
    }
    ...
    const [updatedUser, transaction] = await prisma.$transaction([
      prisma.user.update({
        ...
        data: {
          ...
          zenithBalance: { decrement: coinAmount },
        },
      }),
      prisma.zenithTransaction.create({
        data: {
    ...
    // Log sell
    logAuraCoin('zenith_sell', req.user.id, user.username, {
    ...
    res.status(500).json({ error: 'Failed to sell Zenith' });
  }
});
...
router.get('/transactions/me', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const transactions = await prisma.zenithTransaction.findMany({
...
// Get Zenith leaderboard (top holders)
router.get('/leaderboard', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    console.error('Get Zenith leaderboard error:', error);
