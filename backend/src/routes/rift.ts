...
// Configuration
const INITIAL_PRICE = 210;
const FEE_PERCENTAGE = 0.035; // 3.5% fee
...
const initializePrice = async () => {
  try {
    const latestPrice = await prisma.riftPrice.findFirst({
    ...
    if (latestPrice) {
      ...
      await prisma.riftPrice.create({
        data: { price: INITIAL_PRICE, volume: 0 },
      });
    }
    console.log(`Rift initialized at price: $${currentPrice.toFixed(2)}`);
  } catch (error) {
    console.error('Failed to initialize Rift price:', error);
  }
};
...
export const startPriceEngine = () => {
  ...
  console.log('Rift price engine started');
};
...
export const stopPriceEngine = () => {
  if (priceInterval) {
    ...
    priceInterval = null;
    console.log('Rift price engine stopped');
  }
};
...
router.get('/price', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const history = await prisma.riftPrice.findMany({
      where: {
    ...
    const user = await prisma.user.findUnique({
      ...
      select: { riftBalance: true, money: true },
    });
...
router.post('/buy', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [updatedUser, transaction] = await prisma.$transaction([
      prisma.user.update({
        ...
        data: {
          ...
          riftBalance: { increment: coinsReceived },
        },
      }),
      prisma.riftTransaction.create({
        data: {
          userId: req.user.id,
          type: 'BUY',
    ...
    // Log buy
    logAuraCoin('rift_buy', req.user.id, user.username, {
    ...
    res.json({
      ...
      newBalance: {
        money: updatedUser.money,
        rift: updatedUser.riftBalance,
      },
    });
  } catch (error) {
    console.error('Buy error:', error);
    res.status(500).json({ error: 'Failed to buy Rift' });
  }
});
...
router.post('/sell', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (user.riftBalance < coinAmount) {
      return res.status(400).json({ error: 'Insufficient Rift balance' });
    }
    ...
    const [updatedUser, transaction] = await prisma.$transaction([
      prisma.user.update({
        ...
        data: {
          ...
          riftBalance: { decrement: coinAmount },
        },
      }),
      prisma.riftTransaction.create({
        data: {
    ...
    // Log sell
    logAuraCoin('rift_sell', req.user.id, user.username, {
    ...
    res.status(500).json({ error: 'Failed to sell Rift' });
  }
});
...
// Get Rift leaderboard (top holders)
router.get('/leaderboard', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    console.error('Get Rift leaderboard error:', error);
