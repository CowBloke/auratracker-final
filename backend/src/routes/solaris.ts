...
const initializePrice = async () => {
  try {
    const latestPrice = await prisma.solarisPrice.findFirst({
    ...
    if (latestPrice) {
      ...
      await prisma.solarisPrice.create({
        data: { price: INITIAL_PRICE, volume: 0 },
      });
    }
    console.log(`Solaris initialized at price: $${currentPrice.toFixed(2)}`);
  } catch (error) {
    console.error('Failed to initialize Solaris price:', error);
  }
};
...
export const startPriceEngine = () => {
  ...
  console.log('Solaris price engine started');
};
...
export const stopPriceEngine = () => {
  if (priceInterval) {
    ...
    priceInterval = null;
    console.log('Solaris price engine stopped');
  }
};
...
router.get('/price', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const history = await prisma.solarisPrice.findMany({
      where: {
    ...
    const user = await prisma.user.findUnique({
      ...
      select: { solarisBalance: true, money: true },
    });
...
router.post('/buy', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [updatedUser, transaction] = await prisma.$transaction([
      prisma.user.update({
        ...
        data: {
          ...
          solarisBalance: { increment: coinsReceived },
        },
      }),
      prisma.solarisTransaction.create({
        data: {
          userId: req.user.id,
          type: 'BUY',
    ...
    // Log buy
    logAuraCoin('solaris_buy', req.user.id, user.username, {
    ...
    res.json({
      ...
      newBalance: {
        money: updatedUser.money,
        solaris: updatedUser.solarisBalance,
      },
    });
  } catch (error) {
    console.error('Buy error:', error);
    res.status(500).json({ error: 'Failed to buy Solaris' });
  }
});
...
router.post('/sell', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (user.solarisBalance < coinAmount) {
      return res.status(400).json({ error: 'Insufficient Solaris balance' });
    }
    ...
    const [updatedUser, transaction] = await prisma.$transaction([
      prisma.user.update({
        ...
        data: {
          ...
          solarisBalance: { decrement: coinAmount },
        },
      }),
      prisma.solarisTransaction.create({
        data: {
    ...
    // Log sell
    logAuraCoin('solaris_sell', req.user.id, user.username, {
    ...
    res.status(500).json({ error: 'Failed to sell Solaris' });
  }
});
...
router.get('/transactions/me', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const transactions = await prisma.solarisTransaction.findMany({
...
// Get Solaris leaderboard (top holders)
router.get('/leaderboard', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    console.error('Get Solaris leaderboard error:', error);
