...
const badgeKeySchema = z
  .string()
  .trim()
  .toLowerCase()
  .min(3)
  .max(32)
  .regex(/^[a-z0-9][a-z0-9_-]*$/, 'Key must be lowercase and contain only a-z, 0-9, _ or -');

const badgeStyleSchema = z
  .object({
    type: z.enum(['GRADIENT', 'IMAGE', 'SOLID']).default('GRADIENT'),
    gradient: z.string().min(1).max(400).optional(),
    imageUrl: z.string().min(1).max(500).optional(),
    backgroundColor: z.string().min(1).max(60).optional(),
    textColor: z.string().min(1).max(60).optional(),
    borderColor: z.string().min(1).max(60).optional(),
    borderWidth: z.number().int().min(0).max(8).optional(),
    borderRadius: z.number().int().min(0).max(999).optional(),
    paddingX: z.number().int().min(0).max(16).optional(),
    paddingY: z.number().int().min(0).max(16).optional(),
  ...
  .superRefine((value, ctx) => {
    if (value.type === 'IMAGE') {
      if (!value.imageUrl) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'imageUrl is required for IMAGE badges' });
      } else if (!isAllowedImageUrl(value.imageUrl)) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'imageUrl is not allowed' });
      }
    }

    if (value.type === 'GRADIENT') {
      if (!value.gradient) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'gradient is required for GRADIENT badges' });
      } else if (value.gradient.toLowerCase().includes('url(')) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'gradient cannot contain url()' });
      }
    }
  });

const parseBadgeStyleInput = (input: unknown): Record<string, unknown> => {
...
const safeParseBadgeStyle = (style: string): Record<string, unknown> => {
  try {
...
const toPublicBadge = (badge: { id: string; key: string; name: string; description: string | null; style: string; isActive: boolean }) => ({
  id: badge.id,
  key: badge.key,
  name: badge.name,
  description: badge.description,
  style: safeParseBadgeStyle(badge.style),
...
router.get('/badges', authMiddleware, requireAdmin, async (_req: AuthRequest, res: Response) => {
  try {
...
router.post('/badges', authMiddleware, requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const styleObj = badgeStyleSchema.parse(parseBadgeStyleInput(parsed.style));
    ...
    const badge = await prisma.badge.create({
      ...
      select: { id: true, key: true, name: true, description: true, style: true, isActive: true },
    });

    logAdmin('badge_create', req.user!.id, req.user!.username, badge.id, badge.name, { key: badge.key });

    res.status(201).json({ badge: toPublicBadge(badge) });
  } catch (error) {
...
router.put('/badges/:id', authMiddleware, requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;

    const schema = z.object({
      key: badgeKeySchema.optional(),
    ...
    const parsed = schema.parse(req.body);
    const updateData: { key?: string; name?: string; description?: string | null; style?: string; isActive?: boolean } = {};
    ...
    if (parsed.style !== undefined) {
      const styleObj = badgeStyleSchema.parse(parseBadgeStyleInput(parsed.style));
      updateData.style = JSON.stringify(styleObj);
    }
    ...
    logAdmin('badge_update', req.user!.id, req.user!.username, badge.id, badge.name, { key: badge.key });

    res.json({ badge: toPublicBadge(badge) });
  } catch (error) {
    if (error instanceof z.ZodError) {
      return res.status(400).json({ error: 'Validation failed', details: error.errors });
    }
...
router.delete('/badges/:id', authMiddleware, requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const badge = await prisma.badge.update({
      where: { id },
    ...
    logAdmin('badge_deactivate', req.user!.id, req.user!.username, badge.id, badge.name, { key: badge.key });
    res.json({ success: true });
  } catch (error) {
    console.error('Admin deactivate badge error:', error);
    res.status(500).json({ error: 'Failed to deactivate badge' });
  }
});

// Get a user's badges (admin only)
  ...
  try {
    ...
    const [grants, selections] = await Promise.all([
      prisma.userBadgeGrant.findMany({
        ...
        select: {
          badge: { select: { id: true, key: true, name: true, description: true, style: true, isActive: true } },
          grantedAt: true,
          grantedById: true,
        },
...
router.post('/users/:id/badges/grant', authMiddleware, requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const schema = z.object({ badgeId: z.string().uuid() });
    const { badgeId } = schema.parse(req.body);

    const [user, badge] = await Promise.all([
      prisma.user.findUnique({ where: { id }, select: { id: true, username: true } }),
      prisma.badge.findUnique({ where: { id: badgeId }, select: { id: true, name: true, isActive: true } }),
    ]);

    if (!user) return res.status(404).json({ error: 'User not found' });
    if (!badge || !badge.isActive) return res.status(404).json({ error: 'Badge not found' });

    const grant = await prisma.userBadgeGrant.upsert({
      where: { userId_badgeId: { userId: id, badgeId } },
      create: { userId: id, badgeId, grantedById: req.user!.id },
      update: { revokedAt: null, grantedById: req.user!.id, grantedAt: new Date() },
      select: { id: true },
    });

    logAdmin('badge_grant', req.user!.id, req.user!.username, id, user.username, { badgeId, badgeName: badge.name, grantId: grant.id });
...
router.delete('/users/:id/badges/grant/:badgeId', authMiddleware, requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      await tx.userBadgeGrant.update({
        where: { userId_badgeId: { userId: id, badgeId } },
        data: { revokedAt: new Date() },
      });

      await tx.userBadgeSelection.updateMany({
        where: { userId: id, badgeId },
        data: { badgeId: null },
      });
    });

    logAdmin('badge_revoke', req.user!.id, req.user!.username, id, user.username, { badgeId, badgeName: badge.name });
    res.json({ success: true });
  } catch (error) {
